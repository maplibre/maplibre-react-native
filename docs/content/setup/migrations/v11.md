...
changes to the implementation, the APIs where also improved and are based upon MapLibre GL JS.
...
On Android the `Map` uses `GLSurfaceView` per default instead of `TextureView` for native rendering. This aligns
