...
changes to the implementation, the APIs where also improved and are based upon MapLibre GL.

## Changes to `MapView` Component

### Android View uses `GLSurfaceView` by default

On Android the `MapView` uses `GLSurfaceView` per default instead of `TextureView` for native rendering. This aligns
...
```diff
-import { addCustomHeader, removeCustomHeader, setConnected } from '@maplibre/maplibre-react-native';
...
Also the native setup for iOS is no longer required. `NetworkManager` methods work out of the box.
...
`FillExtrusionLayer`, `RasterLayer`, `BackgroundLayer`) have been consolidated into a single, type-safe `Layer`
component. The layer type is now specified via the required `type` prop:

```diff
-import { FillLayer, LineLayer, CircleLayer } from '@maplibre/maplibre-react-native';
...
The `Layer` component uses TypeScript discriminated unions to ensure type safety. When you specify a `type`, the
`style` prop is automatically typed to match that layer type, and props that don't apply are disallowed:

```tsx
// The style prop is typed as FillLayerStyle
<Layer type="fill" style={{ fillColor: 'blue' }} />

// The style prop is typed as LineLayerStyle
<Layer type="line" style={{ lineWidth: 2 }} />

// BackgroundLayer doesn't support source, sourceLayer, or filter props
// TypeScript will error if you try to use them
<Layer
  type="background"
  style={{ backgroundColor: 'white' }}
  // source="..." // Error: source is not allowed for background layers
/>
```

### Layer Props Types

Individual layer prop types are still exported for use in your own type definitions:
...
The `Animated` namespace continues to provide layer components for animation. These are convenience wrappers around the
unified `Layer` component:

```tsx
import { Animated } from '@maplibre/maplibre-react-native';

// These still work and internally use <Layer type="..." />
<Animated.CircleLayer id="animated-circle" style={{ circleRadius: animatedValue }} />
<Animated.LineLayer id="animated-line" style={{ lineWidth: animatedValue }} />
```
