...
Offline packs are now identified by a UUID-based `id` instead of a user-provided `name`. The `id` is automatically generated when creating a pack:
...
The deprecated `setAccessToken` and `getAccessToken` methods have been removed without replacement. Access tokens should be added through URLs or request headers.
