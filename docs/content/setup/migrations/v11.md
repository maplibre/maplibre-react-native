...
## Changes to `Light` Component

The separate `Light` component has been removed. Light configuration is now passed directly as a prop to `MapView`.
...
The light configuration object structure remains the same - only the way it's passed has changed from a child component to a MapView prop.
