...
## Changes to `ShapeSource` Component

### Props

#### `id` Prop is Now Optional
...
#### `hitbox` Structure Changed

The `hitbox` prop now uses the unified `ViewPadding` type instead of width/height:
...
Also `onPress` now bubbles up to `MapView` by default. To prevent this, call `event.stopPropagation()`.

### Imperative Methods on `ShapeSourceRef`

Several imperative methods have been renamed and simplified:

#### `features()` → `getData()`

The method has been renamed and the return type changed to a `FeatureCollection`:

```diff
-const collection = await shapeSourceRef.current.features(filter);
+const collection = await shapeSourceRef.current.getData(filter);
```
...
#### Cluster Methods Now Use `clusterId` Instead of Feature Object

All cluster-related methods now accept a `clusterId` number instead of a GeoJSON Feature. This is an alignment with MapLibre GL JS. The `clusterId` can be obtained from the `cluster_id` property of a cluster feature:
...
The cluster methods now return `GeoJSON.Feature[]` instead of `GeoJSON.FeatureCollection`:
...
The `LocationManager` now incorporates a `requestPermissions` function, which supports Android and iOS. The standalone `requestAndroidPermissions` function has been removed:
