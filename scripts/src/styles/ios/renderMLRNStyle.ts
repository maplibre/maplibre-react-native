...
const LAYER_TYPES: Record<LayerSpecification["type"] | "light", string> = {
  ...
  "color-relief": "MLNColorReliefStyleLayer",
...
export function renderMLRNStyleH(layers: Layer[], filePath: string): string {
  const layerMethodDecls = layers
    .map(
      (layer) =>
        `- (void)${setLayerMethodName(layer)}:(${LAYER_TYPES[layer.name]} *)layer withReactStyle:(NSDictionary *)reactStyle isValid:(BOOL (^)(void)) isValid;`,
    )
    .join("\n");
...
export function renderMLRNStyle(layers: Layer[], filePath: string): string {
  const dispatchMethods = layers
    ...
    [self set${methodName}:layer withReactStyleValue:styleValue];`;
      ...
      return `- (void)${setLayerMethodName(layer)}:(${LAYER_TYPES[layer.name]} *)layer withReactStyle:(NSDictionary *)reactStyle isValid:(BOOL (^)(void)) isValid
{
