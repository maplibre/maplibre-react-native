...
function androidGetConfigType(androidType: string, prop: any) {
  switch (androidType) {
    case "Integer":
...
export function render(layers: any[], filePath: string): string {
  const layerMethods = layers
    .map((layer: any) => {
      ...
      return `    public static void set${pascalCase(layer.name)}LayerStyle(final ${getLayerType(layer, "android")} layer, MLRNStyle style) {
  ...
  const setterMethods = layers
    .flatMap((layer: any) =>
      layer.properties.flatMap((prop: any) => {
        const layerType = getLayerType(layer, "android");
